...
module namespace headers = 'info:lc/casalini/headers#';
        ...
        if (fn:count($accepts) eq 1) then
            let $a := 
                if (fn:contains($accepts[1], ";")) then
                    ...
                    $accepts[1]
            return 
                if ( fn:matches($a, "application/xml|application/rdf\+xml|text/html") ) then
                    fn:replace($a , '\+' , '%2B')
                else 
                    "application/xml"
        else
            let $accept-values := 
                ...
                return $avalue
            let $valid-accepts := 
                for $a in $accept-values
                where fn:matches($a, "application/xml|application/rdf\+xml|text/html")
                return $a
            return
                if ( fn:count($valid-accepts) > 0 ) then
                    fn:replace($valid-accepts[1] , '\+' , '%2B')
                else 
                    "application/xml"
    return $accept-value
};
